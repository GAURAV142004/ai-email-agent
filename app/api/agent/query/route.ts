...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  // Step 3: Extract Gmail search keywords
...
Keep it simple — 2-4 key words from the query.`,
  ...
  // Step 5: Search each member's Gmail
  const allThreads: any[] = []
  const seenThreadIds = new Set<string>()

  for (const membId of memberIds.slice(0, 5)) {
    try {
      ...
      const listRes = await gmail.users.messages.list({
        userId: 'me', q: gmailQuery, maxResults: 15,
      })
      ...
      for (const msg of (listRes.data.messages ?? []).slice(0, 10)) {
        ...
        try {
          ...
          const content = msgs.map(m => {
            ...
            const from = mh.find(x => x.name === 'From')?.value ?? ''
          ...
          allThreads.push({
            ...
            gmailLink:    `https://mail.google.com/mail/u/0/#search/${encodeURIComponent(gmailQuery)}/${msg.threadId}`,
          })
        } catch { /* skip */ }
      }
    } catch { /* skip member */ }
  ...
  // Step 6: AI analyzes all threads
